...
def different_scales(summary):
    ...
    return { "latency_normal": latency_normal, "latency_bad": latency_bad,
             "tps_low": tps_low, "tps_high": tps_high,
             "gc_bad_events": max(gc_bad_events, 1.0),
             "gc_good_events": 0.0,
...
def compute_harmfulness(summary, scales):
    ...
    for service, vals in summary.items():
        ...
        if higher_worse(latency, scales["latency_normal"], scales["latency_bad"]) is not None:
            latency_harmfulness = higher_worse(latency, scales["latency_normal"], scales["latency_bad"])
        else:
...
STRATEGY_WEIGHTS = {
    ...
    "add_cpu": {"cpu_harmfulness": 0.60, "latency_harmfulness": 0.40},
    ...
    "scale_down": {"idle_harmfulness": 1.00},
...
def print_scales(scales):
    ...
    print(f"Latency (ms): normal <= {scales['latency_normal']}, bad >= {scales['latency_bad']}")
