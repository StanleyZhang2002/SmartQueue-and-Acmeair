...
############ Plan #################
def get_top_action(strategies, min_score=0.3):
    """
    Pick the best strategy with score above min_score for each service. If none, return do_nothing.

    """
    action_plan = {}
    for service, strategy_list in strategies.items():
        if not strategy_list:
            continue
        best_action, score = strategy_list[0]
        if best_action != "do_nothing" and score >= min_score:
            action_plan[service] = best_action
    return action_plan
...
MIN_REPLICA = 1
MAX_REPLICA = 6
EXECUTE_SCRIPT_PATH = "./configuration.sh"
CURRENT_CONFIGURATION = {} # {cpu: ..., mem: ..., replica: ...}


def total_usage(configuration):
    total_cpu = 0
    total_mem = 0
    for conf in configuration.values():
        total_cpu += int(conf.get("cpu", 0))
        total_mem += int(conf.get("memory", 0))
    return total_cpu, total_mem


def make_action(action_plan, action_make=False):
    """
    Apply action according to our action plan only when action is True.
    """
    for service in action_plan.keys():
        if service not in CURRENT_CONFIGURATION:
            CURRENT_CONFIGURATION[service] = {"cpu": 500, "memory": 512, "replica": 1}
    safe_copy = {service: conf.copy() for service, conf in CURRENT_CONFIGURATION.items()}
    for service, action in action_plan.items():    
        configuration = safe_copy[service]
        if action == "add_pods":
            configuration["replica"] = min(MAX_REPLICA, configuration["replica"] + 1)
        elif action == "scale_down":
            if configuration["replica"] > MIN_REPLICA:
                configuration["replica"] = max(MIN_REPLICA, configuration["replica"] - 1)
            else:
                new_cpu = max(MIN_CPU, configuration["cpu"] - 125)
                if new_cpu == configuration["cpu"]:
                    print(f"[SKIP] {service}: scale_down mapped to CPU↓ but already at MIN_CPU={MIN_CPU}m")
                    continue
                print(f"[MAP] {service}: scale_down  CPU {configuration['cpu']}m to {new_cpu}m when replica is 1")
                configuration["cpu"] = new_cpu
        elif action == "add_cpu":
            configuration["cpu"] = max(CPU_CHANGE + configuration["cpu"], MIN_CPU)
        elif action == "add_mem":
            configuration["memory"] = max(MEM_CHANGE + configuration["memory"], MIN_MEM)
        elif action == "tune_threads":
            print(f"[NOTE] {service}: tune_threads suggested cannot be applied at current stage, because we cannot access to the thread in our side.")
            continue
        else:
            print(f"[SKIP] {service}: unknown action {action}")
            continue

        total_cpu, total_mem = total_usage(safe_copy)
        if total_cpu > MAX_LIMIT_CPU or total_mem > MAX_LIMIT_MEM:
            # revert this service change in 'proposed' and warn
            safe_copy[service] = CURRENT_CONFIGURATION[service].copy()
            print(f" Skip {service}:{action} would exceed quota if we do this action according to piazza "
                  f"(After action CPU={total_cpu}m/{MAX_LIMIT_CPU}m, "
                  f"MEM={total_mem}Mi/{MAX_LIMIT_MEM}Mi)")
            continue

        cmd = f'sh {EXECUTE_SCRIPT_PATH} cpu={configuration["cpu"]} memory={configuration["memory"]} replica={configuration["replica"]} service={service}'
        if action_make:
            print("$", cmd)
            subprocess.run(cmd, shell=True, check=False)

        # Update our location configuration with the new changes.
        CURRENT_CONFIGURATION[service] = configuration


def wrap_up_for_a1_part(label, out_csv):
    data = process_data(get_data_from_database())
    data = add_processed_TPS_and_GC(data)
    write_csv_rows(data, out_csv, label)
    summary = per_service_summary(data)
    scales = different_scales()
...
def run_jmeter(load_level):
    """
    Run JMeter script to load data for each iteration of python script, in order to see
    the adpation influences.
    """
    if not load_level:
        print("Must give a load level, small, medium or large")
        return
    script = "./runJmeter.sh"
    if not os.path.exists(script):
        print(f"[WARN] {script} not found; skipping load generation.")
        return
    print(f"[LOAD] running JMeter profile: {load_level}")
    subprocess.run(["bash", script, load_level], check=False)
    print(f"[LOAD] JMeter profile {load_level} finished.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loop", type=int, default=1, help="iterations of Monitor→Analyze→Plan→Execute, if infinity loop, type 0 here")
    parser.add_argument("--interval-sec", type=int, default=30, help="Interval to sleep after one execute of loop to let the adaption update.")
    parser.add_argument("--act", action="store_true", help="actually call configuration.sh (otherwise print only)")
    parser.add_argument("--load", choices=["small", "medium", "large"], help="which JMeter profile to run before each iteration")
    ...
    try:
        while True:
            ...
            current_time = datetime.fromtimestamp(time.time(), tz=timezone.utc).strftime("%Y%m%d-%H%M%S")
            root, ext = os.path.splitext(args.out)
            out_iteration = f"{root}_iteration{iteration}_{current_time}{ext or '.csv'}"
            print(f"\n===== Iteration {iteration}/{args.loop} | label={args.label}-iteration{iteration} =====")
            run_jmeter(args.load)
            strategies = wrap_up_for_a1_part(f"{args.label} iteration{iteration}", out_iteration)
            if args.act:
                plan_to_make = get_top_action(strategies)

                if plan_to_make:
                    print("Current action we plan to make is:", plan_to_make)
                    make_action(plan_to_make, action_make=args.act)
                
                else:
                    print("No action to make!!")
            last_iteration = iteration
            iteration += 1

            if iteration <= args.loop or args.loop == 0:
                print(f"We sleep for {args.interval_sec} until the next iteration happens")
                time.sleep(args.interval_sec)
        if args.act:
            if args.interval_sec > 0:
                print(f"We sleep for {args.interval_sec} to let the last adaptation take effect")
                time.sleep(args.interval_sec)
            run_jmeter(args.load)

            cur_time = datetime.fromtimestamp(time.time(), tz=timezone.utc).strftime("%Y%m%d-%H%M%S")
            rt, ex = os.path.splitext(args.out)
            final_out = f"{rt}_iteration{last_iteration}_postadapt_{cur_time}{ex or '.csv'}"

            wrap_up_for_a1_part(f"{args.label} post-adapt-final", final_out)
